...
def create_chart():
    ...
    canvas.draw()
    canvas.get_tk_widget().grid(row=0,column=0,padx=0,pady=500)
...
image1_label=Label(frame,bg="#1B1B21")  
image1_label.place(x=20,y=5)  # Adjust the coordinates for the image logo

id_label=customtkinter.CTkLabel(frame,font=font2,text='Coffee ID:',text_color='#fff',bg_color='#1B1B21')
id_label.place(x=60,y=75)

id_entry=customtkinter.CTkEntry(frame,font=font2,text_color='#000',fg_color='#fff',border_color='#B2016C')
...
delete_button=customtkinter.CTkButton(frame,command=delete,font=font2,text_color='#fff',text='Delete',fg_color='#DB0B02',hover_color='#8F0600',bg_color='#1B1B21',cursor='hand2',corner_radius=8,width=80)
